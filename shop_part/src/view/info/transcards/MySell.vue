<template>
    <!-- 我卖出的 -->
    <div>
        <div class="goods_box" v-for="item in sellList" :key="item.id" @click="toOrderDetail(item.id)">
            <OrderCardPro :orderInfo="item"/>
        </div>
        <div v-show="(sellList.length !==0)" class="more_btn_box">
            <a-button type="default" class="btn" @click="loadingMore()">加载更多</a-button>
        </div>
        <div v-show="(sellList.length === 0)" class="empty_box">
...
export default{
    ...
    computed:{
        ...mapState({
            sellList: state => state.trade.sellList,
    ...
    methods:{
        ...
        loadingMore(){
            ...
            this.$store.dispatch('reqSellList',this.pageData);
        },
    },
    mounted(){
